...
Public Sub EstadisticasPjs(Userindex As Integer)
    ...
    Else
        conn.Execute "INSERT INTO estadis VALUES('" & UserList(Userindex).Name & "','" & UserList( _
                     Userindex).Stats.GLD & "','" & UserList(Userindex).Stats.Banco & "','" & UserList(Userindex).Remort & _
                     "','" & UserList(Userindex).Stats.MaxHP & "','" & UserList(Userindex).GuildInfo.GuildName & "','" & _
                     UserList(Userindex).Stats.MaxHIT & "','" & UserList(Userindex).Stats.Fama & "','" & UserList( _
                     Userindex).Stats.Elu & "','" & UserList(Userindex).Stats.ELV & "','" & UserList(Userindex).Genero & _
                     "','" & UserList(Userindex).clase & "','" & UserList(Userindex).raza & "','" & UserList( _
                     Userindex).Stats.NPCsMuertos & "')"
        UserList(Userindex).BD = 1
        Call WriteVar(CharPath & Left$(UCase$(UserList(Userindex).Name), 1) & "\" & UCase$(UserList(Userindex).Name) _
                      & ".chr", "INIT", "BD", val(UserList(Userindex).BD))
...
Public Sub BDDConnect()
    ...
    If ServerPrimario = 1 Then
        ...
        conn.ConnectionString = "DRIVER={MySQL ODBC 3.51 Driver};" & "SERVER=localhost;" & "DATABASE=drag;" & _
