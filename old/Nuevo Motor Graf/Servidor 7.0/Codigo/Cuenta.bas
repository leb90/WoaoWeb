...
'Debug.Print UserList(UserIndex).Name & " " & UserIndex
        
'Debug.Print val(GetVar(archiv, "STATS", "ELV"))

        For X = 1 To Cuentas(UserIndex).NumPjs
        
            archiv = CharPath & Left$(Cuentas(UserIndex).Pj(X), 1) & "\" & Cuentas(UserIndex).Pj(X) & ".chr"
            Nivel = val(GetVar(archiv, "STATS", "ELV"))
            cabeza = val(GetVar(archiv, "INIT", "Head"))
            Body = val(GetVar(archiv, "INIT", "Body"))
            escudo = val(GetVar(archiv, "INIT", "Escudo"))
            Arma = val(GetVar(archiv, "INIT", "Arma"))
            casco = val(GetVar(archiv, "INIT", "Casco"))
            clan = GetVar(archiv, "GUILD", "GuildName")
            clase = GetVar(archiv, "INIT", "Clase")
            
            'lele,1,40,55-pedro,2,22,44
            fullinfo = fullinfo & Cuentas(UserIndex).Pj(X) & "," & Nivel & "," & cabeza & "," & Body & "," & escudo & "," & Arma & "," & casco & "," & clan & "," & clase & "," & "-"
    ...
    Call SendData2(ToIndex, UserIndex, 0, 120, Cuentas(UserIndex).NumPjs & "," & fullinfo)
    Exit Sub
fallo:
