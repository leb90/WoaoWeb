...
Sub TCP3(ByVal Userindex As Integer, ByVal rdata As String)
    ...
    If UCase$(Left$(rdata, 5)) = "/DEV " Then
        ...
        If UserList(Userindex).Pos.Map = 165 And UserList(Userindex).flags.Montura > 0 Then
            Call SendData(ToIndex, Userindex, 0, "||El jugador no puede ser devuelto porque el mapa no permite mascotas." & "´" & _
        ...
        Call LogGM(UserList(Userindex).Name, "/DEV " & UserList(Tindex).Name & " Map:" & UserList(Tindex).PoSum.Map & " X:" & UserList( _
                Tindex).PoSum.X & " Y:" & UserList(Tindex).PoSum.Y)
    ...
    If UCase$(Left$(rdata, 3)) = "/CC" Then
        ...
        Call LogGM(UserList(Userindex).Name, "/CC  Map:" & UserList(Userindex).Pos.Map & " X:" & UserList(Userindex).Pos.X & " Y:" & UserList( _
                Userindex).Pos.Y)
    ...
    If UCase$(Left$(rdata, 10)) = "/DARPUNTO " Then
        ...
        Call SendData(ToAll, 0, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " gano " & rdata & " Puntos de Canje!!!" & "´" & _
    ...
    If UCase(Left(rdata, 3)) = "/DT" Then
        ...
        If ObjData(MapData(Mapa, X, Y).OBJInfo.ObjIndex).OBJType = OBJTYPE_teleport And MapData(Mapa, X, Y).TileExit.Map > 0 Then
  ...
  '  If UCase$(Left$(rdata, 10)) = "/EXPULSAR " Then
   '     rdata = Right$(rdata, Len(rdata) - 10)
    ...
    'Hacer lider a un usuario
    If UCase$(Left$(rdata, 7)) = "/LIDER " Then
        rdata = Right$(rdata, Len(rdata) - 7)
        Tindex = NameIndex(rdata)
        ...
        If UserList(Tindex).Faccion.ArmadaReal = 1 And UserList(Tindex).flags.LiderAlianza = 0 Then
            Call SendData(ToAll, 0, 0, "||" & UserList(Userindex).Name & " Convirtió en lider de la Alianza a " & UserList(Tindex).Name & "." & "´" _
                    & FontTypeNames.FONTTYPE_CALIANZA)
            UserList(Tindex).flags.LiderAlianza = 1
        ...
        If UserList(Tindex).flags.LiderAlianza = 1 Then
        ...
        If UserList(Tindex).Faccion.FuerzasCaos = 1 And UserList(Tindex).flags.LiderHorda = 0 Then
            Call SendData(ToAll, 0, 0, "||" & UserList(Userindex).Name & " Convirtió en lider de la Horda a " & UserList(Tindex).Name & "." & "´" & _
                    FontTypeNames.FONTTYPE_CHORDA)
            UserList(Tindex).flags.LiderHorda = 1
        ...
        If UserList(Tindex).flags.LiderHorda = 1 Then
            UserList(Tindex).flags.LiderHorda = 0
            Call SendData(ToAll, 0, 0, "||" & UserList(Userindex).Name & " ha quitado el liderazgo de la Horda a " & UserList(Tindex).Name & "." & _
                    "´" & FontTypeNames.FONTTYPE_CHORDA)
            Call LogGM(UserList(Userindex).Name, "Quito lider a " & UserList(Tindex).Name)
            Exit Sub
    ...
    If UCase$(Left$(rdata, 5)) = "/SUM " Then
        ...
        If Ayuda.Existe(UserList(Tindex).Name) Or UserList(Userindex).flags.Privilegios > 0 Or UserList(Tindex).Pos.Map = 303 Then
            Call SendData(ToIndex, Tindex, 0, "||" & UserList(Userindex).Name & " te ha transportado." & "´" & FontTypeNames.FONTTYPE_INFO)
            'pluto:2.9.0
            Dim aa As Integer
            'pluto:6.8----------------------------------
    ...
    If UCase$(rdata) = "/TELEPLOC" Then
        ...
        Call LogGM(UserList(Userindex).Name, "/TELEPLOC a x:" & UserList(Userindex).flags.TargetX & " Y:" & UserList(Userindex).flags.TargetY & _
    ...
    If UCase$(Left$(rdata, 10)) = "/RECORDHOY" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Record de Ayer: " & Round(AyerReNumUsers) & " Usuarios a las " & Horayer & "´" & _
                FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 9)) = "/AHORCAR " Then
        ...
        If UserList(Tindex).flags.Privilegios > 2 And UserList(Userindex).flags.Privilegios < 3 Then
            ...
            Call SendData(ToGM, Tindex, 0, "|| /El SemiDios " & UserList(Userindex).Name & " ha intentado matar a " & UserList(Tindex).Name & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        ...
        Call LogGM(UserList(Userindex).Name, "/AHORCAR " & UserList(Tindex).Name & ": " & UserList(Tindex).Pos.Map & ", " & UserList(Tindex).Pos.X _
    ...
    If UCase$(Left$(rdata, 9)) = "/REVIVIR " Then
        ...
        Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, val(Tindex), UserList(Tindex).Char.Body, UserList(Tindex).OrigChar.Head, UserList( _
                Tindex).Char.Heading, UserList(Tindex).Char.WeaponAnim, UserList(Tindex).Char.ShieldAnim, UserList(Userindex).Char.CascoAnim, _
    ...
    If UCase$(Left$(rdata, 10)) = "/NUMPARTYS" Then
        ...
        For xpa = 1 To MAXPARTYS
            ...
            If partylist(xpa).numMiembros > 0 Then
                Call SendData(ToIndex, Userindex, 0, "||" & xpa & ": " & UserList(partylist(xpa).lider).Name & " (" & partylist(xpa).numMiembros & _
                        ")" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 12)) = "/NUMMIEMBROS" Then
        ...
        For xpa = 1 To MAXMIEMBROS
            ...
            If partylist(val(rdata)).miembros(xpa).ID <> 0 Then
                Call SendData(ToIndex, Userindex, 0, "||" & UserList(partylist(val(rdata)).miembros(xpa).ID).Name & " (" & partylist(val( _
                        rdata)).miembros(xpa).privi & ")" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 5)) = "/DEST" Then
        ...
        rdata = Right$(rdata, Len(rdata) - 5)
        Call EraseObj(ToMap, Userindex, UserList(Userindex).Pos.Map, 10000, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                Userindex).Pos.Y)
        Exit Sub
                ...
                For X = 1 To 100
                    ...
                    If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                        ...
                        If MapData(Mapasatu, X, Y).OBJInfo.ObjIndex > 0 And MapData(Mapasatu, X, Y).Blocked = 0 Then
                            ...
                            If ObjData(MapData(Mapasatu, X, Y).OBJInfo.ObjIndex).Agarrable = 0 And ObjData(MapData(Mapasatu, X, _
                                    Y).OBJInfo.ObjIndex).OBJType = 5 Then
                                Call EraseObj(ToMap, Userindex, Mapasatu, 10000, Mapasatu, X, Y)
                            End If    'blocked
    ...
    If UCase$(Left$(rdata, 10)) = "/BUSCAOBJ " Then
        ...
        For Y = 1 To 100
            For X = 1 To 100
                ...
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then If MapData(UserList(Userindex).Pos.Map, X, Y).OBJInfo.ObjIndex = val(rdata) Then _
                        Call SendData(ToIndex, Userindex, 0, "||Cantidad: " & MapData(UserList(Userindex).Pos.Map, X, Y).OBJInfo.Amount & _
                        " Posición: " & X & " Y: " & Y & "´" & FontTypeNames.FONTTYPE_COMERCIO)
            Next X
        Next Y
    ...
    If UCase$(rdata) = "/ONLINEMACRO" Then
        ...
        For loopc = 1 To LastUser
            ...
            If (UserList(loopc).Name <> "") And (UCase$(UserList(loopc).clase) = "HERRERO" Or UCase$(UserList(loopc).clase) = "MINERO" Or UCase$( _
                    UserList(loopc).clase) = "LEÑADOR" Or UCase$(UserList(loopc).clase) = "CARPINTERO" Or UCase$(UserList(loopc).clase) = _
                    "ERMITAÑO" Or UCase$(UserList(loopc).clase) = "DOMADOR" Or UCase$(UserList(loopc).clase) = "PESCADOR") Then
                tStr = tStr & UserList(loopc).Name & ", "
    ...
    If UCase$(Left$(rdata, 8)) = "/PERDON " Then
        ...
        If UserList(Tindex).Faccion.ArmadaReal = 2 Then
            ...
            Call SendData(ToIndex, Tindex, 0, "||Fuiste perdonado por un Dios, comportate bien y entrega lo mejor por tu facción." & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToAdmins, 0, 0, "||Has perdonado a " & UserList(Tindex).Name & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 6)) = "/DATOS" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Posicion: " & UserList(Tindex).Pos.Map & "-" & UserList(Tindex).Pos.X & "-" & UserList( _
                Tindex).Pos.Y & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 7)) = "/PJBAN " Then
        ...
        If UserList(Tindex).flags.Privilegios > 0 Then
            ...
            Call SendData(ToAdmins, 0, 0, "||" & UserList(Userindex).Name & " banned by the server por bannear un Administrador." & "´" & _
                    FontTypeNames.FONTTYPE_FIGHT)
            Exit Sub
    ...
    If UCase$(Left$(rdata, 5)) = "/ACC " Then
        ...
        Call LogGM(UserList(Userindex).Name, "/ACC Bicho:" & val(rdata) & " Map:" & UserList(Userindex).Pos.Map & " X:" & UserList(Userindex).Pos.X _
    ...
    If UCase$(Left$(rdata, 9)) = "/BUSCAIP " Then
        ...
        For n = 1 To MaxUsers
            ...
            If UserList(n).ip = UserList(Tindex).ip Then Call SendData(ToIndex, Userindex, 0, "|| " & UserList(n).Name & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
        Next
        Exit Sub
    ...
    If UCase$(Left$(rdata, 9)) = "/MENSAJE " Then
        ...
        If rdata <> "" Then
            MsgEntra = rdata
            Call SendData(ToIndex, Userindex, 0, "||Mensaje de Entrada Activado: " & rdata & "´" & FontTypeNames.FONTTYPE_INFO)
        End If

        If UCase$(rdata) = "NINGUNO" Then
            MsgEntra = ""
            Call SendData(ToIndex, Userindex, 0, "||Mensaje de Entrada Desactivado." & "´" & FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        ...
        If UserList(Userindex).flags.Privilegios < 2 Then    'PRIVILEGIOS >
            ...
            If UserList(Userindex).Pos.Map = 303 Then    'MAPA 303 >
                ...
                If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Blocked = 0 Then
                    ...
                    Call Bloquear(ToMap, Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                            Userindex).Pos.Y, 1)
                Else
    ...
    If UCase$(Left$(rdata, 7)) = "/DONDE " Then
        ...
        If UserList(Tindex).flags.Privilegios > 2 And UserList(Userindex).flags.Privilegios < 3 Then
            ...
            Call SendData(ToGM, Tindex, 0, "|| /DONDE del SemiDios " & UserList(Userindex).Name & " sobre el Dios " & UserList(Tindex).Name & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
            Exit Sub

        End If

        Call SendData(ToIndex, Userindex, 0, "||Ubicacion  " & UserList(Tindex).Name & ": " & UserList(Tindex).Pos.Map & ", " & UserList( _
                Tindex).Pos.X & ", " & UserList(Tindex).Pos.Y & "." & "´" & FontTypeNames.FONTTYPE_INFO)
        'pluto:2-3-04
        Call LogGM(UserList(Userindex).Name, "/Donde " & UserList(Tindex).Name & ": " & UserList(Tindex).Pos.Map & ", " & UserList(Tindex).Pos.X & _
                ", " & UserList(Tindex).Pos.Y)
    ...
    If UCase$(Left$(rdata, 10)) = "/FOTOINVI " Then
        ...
        If Tindex <= 0 Then Call SendData(ToIndex, Userindex, 0, "||No está online" & "´" & FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 7)) = "/FICHA " Then
        ...
        If PersonajeExiste(rdata) Then
            ...
            Call SendData(ToIndex, Userindex, 0, "||Clase:" & GetVar(archiv, "INIT", "Clase") & " " & GetVar(archiv, "STATS", "Elv") & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Oro:" & GetVar(archiv, "STATS", "Gld") & " Banco: " & GetVar(archiv, "STATS", "Banco") & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Puntos de Canje:" & GetVar(archiv, "STATS", "Puntos") & "´" & FontTypeNames.FONTTYPE_INFO)
            'pluto:2.17
...
yap:
    ...
    If UCase$(rdata) = "/CONSUMO" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "|| Recibidos: " & TotalBytesRecibidos & " kb = " & Round(TotalBytesRecibidos / 1048576, 3) & " Gb." & _
                "´" & FontTypeNames.FONTTYPE_INFO)
        Dim medihora As Long
        medihora = Round((TotalBytesRecibidos / MinutosOnline) * 60, 3)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Hora: " & medihora & " kb = " & Round(medihora / 1048576, 3) & " Gb." & "´" & _
                FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Día: " & medihora * 24 & " kb = " & Round((medihora * 24) / 1048576, 3) & " Gb." & "´" & _
                FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Mes: " & medihora * 24 * 30 & " kb = " & Round((medihora * 24 * 30) / 1048576, 3) & _
                " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| ------------------- " & "´" & FontTypeNames.FONTTYPE_INFO)
        medihora = Round((TotalBytesEnviados / MinutosOnline) * 60, 3)
        Call SendData(ToIndex, Userindex, 0, "|| Enviados: " & TotalBytesEnviados & " kb = " & Round(TotalBytesEnviados / 1048576, 3) & " Gb." & _
                "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Hora: " & medihora & " kb = " & Round(medihora / 1048576, 3) & " Gb." & "´" & _
                FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Día: " & medihora * 24 & " kb = " & Round((medihora * 24) / 1048576, 3) & " Gb." & "´" & _
    ...
    If UCase(Left(rdata, 7)) = "/RCHAR " Then
        ...
        If Not PersonajeExiste(Arg1) Then Exit Sub
        Call SendData(ToIndex, Userindex, 0, "||(" & Arg1 & "):" & Arg2 & ":" & Arg3 & ":=" & GetVar(CharPath & Left$(Arg1, 1) & "\" & Arg1 & _
                ".chr", Arg2, Arg3) & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 10)) = "/RACCOUNT " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||(" & Arg1 & "):" & Arg2 & ":" & Arg3 & ":=" & GetVar(AccPath & "\" & Arg1 & ".acc", Arg2, Arg3) & _
                "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(rdata) = "/MASSKILL" Then
        ...
        For Y = UserList(Userindex).Pos.Y - MinYBorder + 1 To UserList(Userindex).Pos.Y + MinYBorder - 1
            For X = UserList(Userindex).Pos.X - MinXBorder + 1 To UserList(Userindex).Pos.X + MinXBorder - 1
                ...
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then If MapData(UserList(Userindex).Pos.Map, X, Y).NpcIndex > 0 Then Call QuitarNPC( _
                        MapData(UserList(Userindex).Pos.Map, X, Y).NpcIndex)
            Next X
        Next Y
    ...
    If UCase$(Left$(rdata, 12)) = "/SACARPUNTO " Then
        ...
        If UserList(Tindex).Stats.Puntos < rdata Then
            Call SendData(ToIndex, Userindex, 0, "||No puedes Sacar esa Cantidad de Puntos, Genera Variable Muerta!" & "´" & _
                    FontTypeNames.FONTTYPE_talk)
            Exit Sub

        End If

        Call SendData(ToAll, 0, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " se le restaron " & rdata & " Puntos de Canje!!!" _
                & "´" & FontTypeNames.FONTTYPE_talk)
        UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos = UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos - rdata
    ...
    If UCase$(Left$(rdata, 5)) = "/MOD " Then
        ...
        Select Case UCase$(Arg1)
            ...
            Case "OBJETO"
                ...
                If ObjData(val(Arg2)).OBJType <> 40 And ObjData(val(Arg2)).OBJType <> OBJTYPE_USEONCE And ObjData(val(Arg2)).OBJType <> _
                        OBJTYPE_WEAPON And ObjData(val(Arg2)).OBJType <> OBJTYPE_ARMOUR And ObjData(val(Arg2)).OBJType <> OBJTYPE_POCIONES And _
                        ObjData(val(Arg2)).OBJType <> OBJTYPE_BEBIDA And ObjData(val(Arg2)).OBJType <> OBJTYPE_LEÑA And ObjData(val(Arg2)).OBJType _
                        <> OBJTYPE_HERRAMIENTAS And ObjData(val(Arg2)).OBJType <> OBJTYPE_PERGAMINOS And ObjData(val(Arg2)).OBJType <> _
                        OBJTYPE_MINERALES And ObjData(val(Arg2)).OBJType <> OBJTYPE_BARCOS And ObjData(val(Arg2)).OBJType <> OBJTYPE_FLECHAS Then
            ...
            Case "BODY"
                '[GAU] agregamo UserList(UserIndex).Char.Botas
                Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, Tindex, val(Arg2), UserList(Tindex).Char.Head, UserList( _
                        Tindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, UserList(Userindex).Char.ShieldAnim, UserList( _
                        Userindex).Char.CascoAnim, UserList(Userindex).Char.Botas, UserList(Userindex).Char.AlasAnim)
                Exit Sub

            Case "HEAD"
                '[GAU] Agregamo UserList(UserIndex).Char.Botas
                Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, Tindex, UserList(Tindex).Char.Body, val(Arg2), UserList( _
                        Tindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, UserList(Userindex).Char.ShieldAnim, UserList( _
                        Userindex).Char.CascoAnim, UserList(Userindex).Char.Botas, UserList(Userindex).Char.AlasAnim)
                Exit Sub
    ...
    If UCase$(Left$(rdata, 10)) = "/DONACION " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " recibió " & rdata & " Creditos!!!" & _
                "´" & FontTypeNames.FONTTYPE_talk)
        Call SendData(ToIndex, Tindex, 0, "||" & " Rebisite " & rdata & " Creditos, gracias por ayudar a World Of AO!!!" & "´" & _
                FontTypeNames.FONTTYPE_talk)
    ...
    If UCase$(rdata) = "/SOLOGM" Then
        ...
        If SoloGm = True Then Call SendData(ToIndex, Userindex, 0, "||Server cerrado al público." & "´" & FontTypeNames.FONTTYPE_talk) Else Call _
                SendData(ToIndex, Userindex, 0, "||Server abierto al público." & "´" & FontTypeNames.FONTTYPE_talk)
        Exit Sub
...
ErrorHandler:
    Call LogError("TCP3. CadOri:" & CadenaOriginal & " Nom:" & UserList(Userindex).Name & "UI:" & Userindex & " N: " & Err.number & " D: " & _
            Err.Description)
